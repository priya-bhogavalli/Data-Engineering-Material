# Python Resources

## Official Documentation
- **Python.org**: https://docs.python.org/3/
- **Python Package Index (PyPI)**: https://pypi.org/
- **Python Enhancement Proposals (PEPs)**: https://peps.python.org/
- **Python Developer's Guide**: https://devguide.python.org/

## Data Engineering Libraries

### Data Processing
- **Pandas**: https://pandas.pydata.org/ - Data manipulation and analysis
- **NumPy**: https://numpy.org/ - Numerical computing
- **Dask**: https://dask.org/ - Parallel computing
- **Polars**: https://pola.rs/ - Fast DataFrame library
- **Vaex**: https://vaex.io/ - Out-of-core DataFrame library

### Database Connectivity
- **SQLAlchemy**: https://sqlalchemy.org/ - SQL toolkit and ORM
- **psycopg2**: https://pypi.org/project/psycopg2/ - PostgreSQL adapter
- **PyMySQL**: https://pypi.org/project/PyMySQL/ - MySQL client
- **cx_Oracle**: https://oracle.github.io/python-cx_Oracle/ - Oracle database

### Big Data & Spark
- **PySpark**: https://spark.apache.org/docs/latest/api/python/ - Apache Spark Python API
- **PyArrow**: https://arrow.apache.org/docs/python/ - Columnar data processing
- **Koalas**: https://koalas.readthedocs.io/ - Pandas API on Apache Spark

### Cloud Services
- **boto3**: https://boto3.amazonaws.com/v1/documentation/api/latest/index.html - AWS SDK
- **google-cloud**: https://cloud.google.com/python/docs - Google Cloud SDK
- **azure-sdk**: https://docs.microsoft.com/en-us/azure/developer/python/ - Azure SDK

### Workflow Orchestration
- **Apache Airflow**: https://airflow.apache.org/ - Workflow management
- **Prefect**: https://prefect.io/ - Modern workflow orchestration
- **Dagster**: https://dagster.io/ - Data orchestrator

### API Development
- **FastAPI**: https://fastapi.tiangolo.com/ - Modern web framework
- **Flask**: https://flask.palletsprojects.com/ - Micro web framework
- **Django REST**: https://www.django-rest-framework.org/ - REST API framework

## Learning Resources

### Books
- **"Effective Python" by Brett Slatkin** - Best practices
- **"Python Tricks" by Dan Bader** - Advanced techniques
- **"Fluent Python" by Luciano Ramalho** - Deep dive into Python
- **"Data Engineering with Python" by Paul Crickard** - DE-specific
- **"Python for Data Analysis" by Wes McKinney** - Pandas creator's guide

### Online Courses
- **Real Python**: https://realpython.com/ - Comprehensive tutorials
- **Python.org Tutorial**: https://docs.python.org/3/tutorial/
- **Coursera Python Specializations**: Various university courses
- **edX Python Courses**: MIT and other institutions
- **Udemy Python Courses**: Practical, project-based learning

### Practice Platforms
- **LeetCode**: https://leetcode.com/ - Algorithm practice
- **HackerRank**: https://hackerrank.com/ - Programming challenges
- **Codewars**: https://codewars.com/ - Coding kata
- **Project Euler**: https://projecteuler.net/ - Mathematical problems

### YouTube Channels
- **Corey Schafer**: Python tutorials and best practices
- **Real Python**: Video tutorials and tips
- **Tech With Tim**: Python projects and tutorials
- **Sentdex**: Python for data science and ML

## Development Tools

### IDEs and Editors
- **PyCharm**: https://jetbrains.com/pycharm/ - Full-featured IDE
- **VS Code**: https://code.visualstudio.com/ - Lightweight editor
- **Jupyter**: https://jupyter.org/ - Interactive notebooks
- **Spyder**: https://spyder-ide.org/ - Scientific Python IDE

### Code Quality
- **Black**: https://black.readthedocs.io/ - Code formatter
- **flake8**: https://flake8.pycqa.org/ - Style guide enforcement
- **pylint**: https://pylint.org/ - Code analysis
- **mypy**: https://mypy.readthedocs.io/ - Static type checker
- **pre-commit**: https://pre-commit.com/ - Git hooks

### Testing
- **pytest**: https://pytest.org/ - Testing framework
- **unittest**: Built-in testing framework
- **coverage.py**: https://coverage.readthedocs.io/ - Code coverage
- **tox**: https://tox.readthedocs.io/ - Testing automation

### Package Management
- **pip**: Built-in package installer
- **conda**: https://conda.io/ - Package and environment manager
- **poetry**: https://python-poetry.org/ - Dependency management
- **pipenv**: https://pipenv.pypa.io/ - Package management

## Community and Support

### Forums and Communities
- **Stack Overflow**: https://stackoverflow.com/questions/tagged/python
- **Reddit r/Python**: https://reddit.com/r/Python/
- **Python Discord**: https://discord.gg/python
- **Python Forum**: https://discuss.python.org/

### Conferences
- **PyCon**: https://pycon.org/ - Main Python conference
- **EuroPython**: https://europython.eu/ - European Python conference
- **PyData**: https://pydata.org/ - Data science focused
- **DjangoCon**: https://djangocon.us/ - Django framework conference

### Newsletters and Blogs
- **Python Weekly**: https://pythonweekly.com/
- **Real Python Newsletter**: Weekly Python tips
- **Planet Python**: https://planetpython.org/ - Blog aggregator
- **Python Software Foundation Blog**: https://pyfound.blogspot.com/

## Certification and Career

### Certifications
- **Python Institute Certifications**: PCAP, PCPP
- **Microsoft Python Certification**: Azure-focused
- **Google Python Certification**: Cloud-focused

### Job Resources
- **Python Jobs**: https://pythonjobs.github.io/
- **Stack Overflow Jobs**: Python-specific listings
- **AngelList**: Startup Python positions
- **LinkedIn**: Professional networking

## Data Engineering Specific Resources

### Blogs and Articles
- **Towards Data Science**: Medium publication
- **Data Engineering Weekly**: Newsletter
- **The Data Engineering Podcast**: Industry insights
- **AWS Big Data Blog**: Cloud data engineering

### Tools and Frameworks
- **Apache Kafka**: Stream processing
- **Apache Flink**: Real-time processing
- **dbt**: Data transformation
- **Great Expectations**: Data quality
- **MLflow**: ML lifecycle management

### Cloud Platforms
- **AWS Data Services**: S3, Glue, EMR, Redshift
- **Google Cloud Data**: BigQuery, Dataflow, Composer
- **Azure Data Services**: Data Factory, Synapse, Databricks

## Version-Specific Resources

### Python 3.12 (Latest)
- **What's New**: https://docs.python.org/3.12/whatsnew/3.12.html
- **New Features**: Pattern matching improvements, better error messages

### Python 3.11
- **Performance Improvements**: 10-60% faster than 3.10
- **Better Error Messages**: More precise tracebacks

### Python 3.10
- **Structural Pattern Matching**: match/case statements
- **Union Types**: X | Y syntax

### Python 3.9
- **Dictionary Merge Operators**: | and |=
- **Type Hinting Improvements**: Built-in generics

### Python 3.8
- **Walrus Operator**: := assignment expressions
- **Positional-Only Parameters**: / syntax

## Quick Reference Links
- **Python Cheat Sheet**: https://quickref.me/python
- **Python Module Index**: https://docs.python.org/3/py-modindex.html
- **Python Standard Library**: https://docs.python.org/3/library/
- **Python HOWTOs**: https://docs.python.org/3/howto/